...
(function() {
  ...
  exports.Builder = (function() {
    ...
    Builder.prototype.buildObject = function(rootObj) {
      var attrkey, charkey, render, rootElement, rootName, preserveChildrenOrder, childkey;
      attrkey = this.options.attrkey;
      charkey = this.options.charkey;
      preserveChildrenOrder = this.options.preserveChildrenOrder;
          ...
          if (typeof obj !== 'object') {
            ...
            for (key in obj) {
              ...
              if (key === attrkey) {
                ...
                // HACK: specially handle $$
                if(preserveChildrenOrder){
                  if(key != childkey) continue;
                  for (index in child) {
                    if (!hasProp.call(child, index)) continue;
                    entry = child[index];
                    var hackKey = entry["#name"];
                    if (typeof entry === 'string') {
                      if (_this.options.cdata && requiresCDATA(entry)) {
                        element = element.ele(hackKey).raw(wrapCDATA(entry)).up();
                      } else {
                        // HACK: omit #name
                        if(key.charAt(0) == "#") continue;
                        element = element.ele(hackKey, entry).up();
                      }
                    } else {
                      element = arguments.callee(element.ele(hackKey), entry).up();
                    }
                  }
                // HACK: this is the normal route
                }else{
                  for (index in child) {
                    if (!hasProp.call(child, index)) continue;
                    entry = child[index];
